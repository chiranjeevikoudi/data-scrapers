...
var nameLinksFile = "./data/name_links_sample_data.json";
...
if(WRITE_TO === "FILE"){
    if(!(FILE_NAME && FILE_NAME.indexOf(".json")>-1 && FILE_NAME.length-5 === FILE_NAME.indexOf(".json") )){
        FILE_NAME = "./data/name_gender_country_sample_data.json";
    }
}
            ...
            if(err){
                ...
                async.each(nameLinks,function (nameLink,callback) {
                    ...
                    getNamesInCountry(countryName,nameLink,function (err,names) {
                        console.log(countryName+" country data scraping completed");
                        if(err){
                            callback(err);
                        }
                        else if(names){
